...
Triggers.MechTTL = [Trig(1).info{1}];
Triggers.Laser = [Trig(2).info{1}];
% Triggers.CED = [Trig(3).info{1}];
% Triggers.LFP = [Trig(4).info{1}];
% Triggers.Resp = [Trig(5).info{1}];
Triggers.MechStim = [Trig(6).info{1}];
for a = 2: lngth
    Triggers.MechTTL = [Triggers.MechTTL; Trig(1).info{a}];
...
%     Triggers.Resp = [Triggers.Resp; Trig(5).info{a}];
    Triggers.MechStim = [Triggers.MechStim; Trig(6).info{a}];
end
