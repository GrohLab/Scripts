...
[f, z_axis, poly_coords] = createPolarPlotPolygons( med_AI_all' );
...
arrayfun(@(v, y) text( real( 1.25 * max( poly_coords(v,:) ) ), ...
    imag( 1.25 * max( poly_coords(v,:) ) ), sprintf("$p=%.3g$", p(v) ), ...
    "HorizontalAlignment", "center", "Rotation", y, ...
    "VerticalAlignment", "baseline", "Interpreter", "latex"), 1:8, ...
    (180*angle( transp(z_axis) )/pi) - 90 );
