...
for cad = tocol(animalFolders(~exclude_flags))'
    ...
    for csd = sessDirs(:)'
        ...
        data_path = curDir;
        fprintf(1, ', Session %s\n', currSess )
        rfpStruct = dir( fullfile( data_path, "Regression " + ...
            "CW-800.00-800.00ms DW-100.00-100.00 BZ5.00.mat" ) );
        if ~isempty(rfpStruct)
            try
                load( expandName( rfpStruct ), "mdlAll_ind", "DX", "params" )
                mdl = mdlAll_ind;
            catch ME
                display(ME.message)
                continue
        ...
        y_trials_pre = getTimeBoAT(y_ltrials, laserFlags, params);
        y_ptrials_pre = getTimeBoAT(y_lptrials, laserFlags, params);
        r_sq_lpre = goodnessFit2(y_trials_pre, y_ptrials_pre, 1);

        y_trials_post = getTimeBoAT( y_ltrials, ~laserFlags, params );
        y_ptrials_post = getTimeBoAT( y_lptrials, ~laserFlags, params );
        r_sq_lpost = goodnessFit2(y_trials_post, y_ptrials_post, 1);
        
        rmse_laser = getRMSE( DX{4}, y_lpred, 1 );

        dataTable = table( {[r_sq;r_sq_pre;r_sq_post],[r_sq_l;r_sq_lpre;r_sq_lpost]}, ...
