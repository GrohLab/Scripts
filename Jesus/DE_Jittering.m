...
for ccond = 1:Nccond
    figFileName = sprintf('%s %s VW%.1f-%.1f ms B%.1f ms RW%.1f-%.1f ms SW%.1f-%.1f ms %sset %s (%s)',...
        expName, Conditions(consideredConditions(ccond)).name, timeLapse(1)*1e3,...
        timeLapse(2)*1e3, binSz*1e3, responseWindow(1)*1e3, responseWindow(2)*1e3,...
        spontaneousWindow(1)*1e3, spontaneousWindow(2)*1e3, onOffStr,...
        orderedStr, filtStr);
