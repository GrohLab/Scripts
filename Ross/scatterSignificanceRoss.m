...
for cr = 1:Nr
    ...
    while csp <= figType
        ...
        line(ax(csp), 'XData', [0, axMx],...
            ...
            'DisplayName', 'y = x');
        title(ax(csp),ttle, 'Interpreter', 'none'); 
        xlabel(ax(csp), [axLabels{aslSubX},CondNames{cond1},'} [Hz]'],  'Interpreter', 'none'); 
        ylabel(ax(csp), [axLabels{aslSubY},CondNames{cond2},'} [Hz]'],  'Interpreter', 'none');
