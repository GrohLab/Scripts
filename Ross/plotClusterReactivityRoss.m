function fig =...
    plotClusterReactivityRoss(PSTH,trig,sweeps,timeLapse,binSz,IDe,expName,stims,IDs)
...
% ax1.YAxis.Label.String = fprintf('Unit Depth_{(\mum)', 'Interpreter', 'tex');
ax1.XAxis.Visible = 'off';
ax1.FontSize = 30;
ax1.FontName = 'Arial';
ylabel('Unit Depth_{(\mum)}', 'Interpreter', 'tex')
...
if fthAxFlag
    ...
    for cs = 1:min(r,c)
        if exist('IDs','var')
            plot(ax2,trigTX,stims(:,cs),'LineStyle','-','LineWidth',2,...
                'DisplayName', IDs{cs})
        else
            plot(ax2,trigTX,stims(:,cs),'LineStyle','-','LineWidth',2)
        end
